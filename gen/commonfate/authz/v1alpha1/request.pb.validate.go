...
func (m *Request) validate(all bool) error {
	...
	for idx, item := range m.GetOverlayEntities() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, RequestValidationError{
						field:  fmt.Sprintf("OverlayEntities[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
	...
	for idx, item := range m.GetOverlayChildren() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, RequestValidationError{
						field:  fmt.Sprintf("OverlayChildren[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}

...
func (m *ProvisionRequest) validate(all bool) error {
	...
	if all {
		switch v := interface{}(m.GetIntegrationConfig()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
				errors = append(errors, ProvisionRequestValidationError{
	...
	} else if v, ok := interface{}(m.GetIntegrationConfig()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
			return ProvisionRequestValidationError{
				field:  "IntegrationConfig",
				reason: "embedded message failed validation",
	...
	for idx, item := range m.GetAdditionalIntegration() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, ProvisionRequestValidationError{
						field:  fmt.Sprintf("AdditionalIntegration[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
